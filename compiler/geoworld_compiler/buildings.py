...
class BuildingSource:
    ...
    def __init__(self, osm_json_path: str | Path | None,
        ...
        def _band(r0, r1, w0, w1, wt):
            ...
            if sub is not None:
                ...
                win_bus[sr0:sr1, sc0:sc1] = bus_lut[win_idx]
                ...
                # win_idx is sub-window-local, so cell centers use sub_t.
                sub_zone = np.zeros((sr1 - sr0, sc1 - sc0), dtype=np.uint8)
                for idx in np.unique(win_idx):
                    ...
                    es = sub_t.c + (cols + 0.5) * sub_t.a
                    ns = sub_t.f + (rows + 0.5) * sub_t.e
                    sub_zone[mask] = zone_field(inst, es, ns)
                win_zone[sr0:sr1, sc0:sc1] = sub_zone
    ...
    def business_grid(self, east: np.ndarray, north: np.ndarray) -> np.ndarray:
        """(H, W) u8 business registry ids for cell-center vectors."""
        return np.asarray(self._gather(self._business.grid, east, north, 0),
                          dtype=np.uint8)

    def interior_zone_grid(self, east: np.ndarray,
                           north: np.ndarray) -> np.ndarray:
        """(H, W) u8 interior zone ids for cell-center vectors."""
        return np.asarray(self._gather(self._interior.grid, east, north, 0),
                          dtype=np.uint8)
